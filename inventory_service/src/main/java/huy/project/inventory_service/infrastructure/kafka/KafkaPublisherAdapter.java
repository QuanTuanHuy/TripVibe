...
import huy.project.inventory_service.core.port.IKafkaPublisher;
...
public class KafkaPublisherAdapter implements IKafkaPublisher {
    ...
    @Override
    public <T> void sendMessageAsync(T data, String topic, String kafkaServer, KafkaOperationCallback callback) {
        String message;
        try {
    ...
    public <T> void pushAsync(T payload, String topic, String kafkaServer) {
        if (!StringUtils.hasText(kafkaServer)) {
            kafkaServer = defaultKafkaServer.getBootstrapServers();
        }
