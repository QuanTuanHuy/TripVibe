...
func (h *MockTaskHandler) Handle(ctx context.Context, task *entity.Task) error {
	h.mu.Lock()
	defer h.mu.Unlock()
...
func TestTaskWorker_RegisterHandler(t *testing.T) {
	...
	logger := zap.NewNop()
	repo := task.NewRedisTaskRepo(client, logger)
	queue := task.NewRedisTaskQueue(client, logger, "test-queue", 100)
...
func TestTaskWorker_ProcessTask(t *testing.T) {
	...
	testTask := &entity.Task{
		...
		Name:       "Test Task",
		Type:       "mock",
		Priority:   entity.TaskPriorityNormal,
	...
	require.NoError(t, err)
	assert.Equal(t, entity.TaskStatusCompleted, retrievedTask.Status)
}
...
func TestTaskWorker_ProcessTaskWithError(t *testing.T) {
	...
	retrievedTask, err := repo.GetByID(ctx, testTask.ID)
	require.NoError(t, err)
	assert.Equal(t, entity.TaskStatusFailed, retrievedTask.Status)
