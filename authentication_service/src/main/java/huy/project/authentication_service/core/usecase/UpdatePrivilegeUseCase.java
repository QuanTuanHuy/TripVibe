...
@Slf4j
public class UpdatePrivilegeUseCase {
    ...
    public PrivilegeEntity updatePrivilege(Long id, UpdatePrivilegeRequestDto req) {
        Pair<Boolean, PrivilegeEntity> privilegeExist = privilegeValidation.isPrivilegeExist(id);
        if (!privilegeExist.getFirst()) {
            log.error("Privilege not found");
            throw new AppException(ErrorCode.PRIVILEGE_NOT_FOUND);
        }

        if (!req.getName().equals(privilegeExist.getSecond().getName()) && privilegeValidation.isPrivilegeNameExist(req.getName())) {
            log.error("Privilege name existed");
            throw new AppException(ErrorCode.PRIVILEGE_NAME_EXISTED);
        }

        PrivilegeEntity privilege = privilegeExist.getSecond();
        privilege = PrivilegeMapper.INSTANCE.toEntity(privilege, req);
